...
function runSeed() {
  ...
  // Seed Default Policy
  try {
    const policyStmt = db.prepare('INSERT OR REPLACE INTO policies (merchant_id, max_discount_percentage, max_automatic_amount, max_retry_attempts) VALUES (?, ?, ?, ?)');
    policyStmt.run('merch_001', 10.0, 10000.0, 3);
  } catch (e) {
    try {
      db.prepare('INSERT OR REPLACE INTO policies (merchant_id, max_discount_percentage, max_automatic_amount, max_retry_attempts, cooldown_period_hours) VALUES (?, ?, ?, ?, ?)').run('merch_001', 10.0, 10000.0, 3, 24);
    } catch (e2) {}
  }
  ...
  const insertProd = db.prepare(`
    INSERT OR REPLACE INTO products (id, merchant_id, name, price, stock, category, sku, image, description, agent_readiness, status, json_ld, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
